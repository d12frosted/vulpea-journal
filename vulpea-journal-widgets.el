      ...
      (while (<= day days-in-month)
        (let* ((is-today (and (= day today-day)
               ...
               (cell (if is-current
                       ...
                       (concat " " (propertize day-str 'face face) " "))))
          ;; Make day clickable and navigable
...
(defun vulpea-journal--in-calendar-p ()
  "Return non-nil if point is in a calendar cell."
  (get-text-property (point) 'calendar-cell))

(defun vulpea-journal--find-calendar-cell (direction)
  "Find next calendar cell in DIRECTION (:left :right :up :down).
Returns position or nil."
  (let ((col (current-column))
        (found nil)
        (current-date (get-text-property (point) 'action-data)))
    (save-excursion
      (pcase direction
        (:left
         (when (> (point) (point-min))
           (backward-char 1)
           ;; Find cell with different date
           (while (and (not (bobp))
                       (not found))
             (let ((cell-date (get-text-property (point) 'action-data)))
               (when (and cell-date
                          (get-text-property (point) 'calendar-cell)
                          (not (equal cell-date current-date)))
                 (setq found (point))))
             (unless found (backward-char 1)))))
        (:right
        ...
        (:up
         (forward-line -1)
         (move-to-column col)
         ;; Find nearest cell on this line
         (let ((line-start (line-beginning-position)))
           (when (get-text-property (point) 'calendar-cell)
             (setq found (point)))
           (unless found
             ;; Search nearby
             (let ((search-pos (point)))
               (while (and (not found) (>= search-pos line-start))
                 (when (get-text-property search-pos 'calendar-cell)
                   (setq found search-pos))
        ...
        (:down
         ...
         (let ((line-end (line-end-position)))
           ...
           (unless found
             ;; Search nearby
             (let ((search-pos (point)))
               (while (and (not found) (<= search-pos line-end))
                 (when (get-text-property search-pos 'calendar-cell)
                   (setq found search-pos))
                 (setq search-pos (1+ search-pos)))))))))
    found))

(defun vulpea-journal-calendar-left ()
  "Move to previous calendar cell, or default movement."
  (interactive)
  (if (vulpea-journal--in-calendar-p)
      (when-let ((pos (vulpea-journal--find-calendar-cell :left)))
        (goto-char pos))
    (backward-char 1)))

(defun vulpea-journal-calendar-right ()
  "Move to next calendar cell, or default movement."
  (interactive)
  (if (vulpea-journal--in-calendar-p)
      (when-let ((pos (vulpea-journal--find-calendar-cell :right)))
        (goto-char pos))
    (forward-char 1)))

(defun vulpea-journal-calendar-up ()
  "Move to calendar cell above, or default movement."
  (interactive)
  (if (vulpea-journal--in-calendar-p)
      (if-let ((pos (vulpea-journal--find-calendar-cell :up)))
          (goto-char pos))
    (forward-line -1)))

(defun vulpea-journal-calendar-down ()
  "Move to calendar cell below, or default movement."
  (interactive)
  (if (vulpea-journal--in-calendar-p)
      (if-let ((pos (vulpea-journal--find-calendar-cell :down)))
          (goto-char pos))
    (forward-line 1)))
