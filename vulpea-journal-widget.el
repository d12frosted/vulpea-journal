...
(defun vulpea-journal-toggle-widget ()
  "Toggle widget at point, or move to next actionable element."
  (interactive)
  (if-let ((name (get-text-property (point) 'widget-name)))
      (vulpea-journal-toggle-widget-by-name name)
    (vulpea-journal-next-action)))

(defun vulpea-journal-next-action ()
  "Move to next actionable element (button, link, calendar day)."
  (interactive)
  (let ((found nil)
        (start (point)))
    (save-excursion
...
(defun vulpea-journal-prev-action ()
  "Move to previous actionable element (button, link, calendar day)."
  (interactive)
  (let ((found nil)
        (start (point)))
    (save-excursion
...
(with-eval-after-load 'vulpea-journal
  ...
  (define-key vulpea-journal-widgets-mode-map (kbd "<backtab>") #'vulpea-journal-prev-action))
